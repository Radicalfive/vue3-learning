<template>
  <div>
    <h2 style="color: #49aaf8">VuexCpnOne</h2>
    <!-- <div>计数值：{{ counter }}</div> -->
    <!-- 1. counter值不同步 -->
    ...
    <!-- 5. 通过payload封装多个参数 -->
    <!-- <div>计数值：{{ $store.state.counter }}</div>
    <button @click="$store.commit('add', { num: 5, multiple: 2 })">+10</button>
    <button @click="$store.commit('sub', { num: 5, multiple: 2 })">-10</button> -->

    <!-- 6.使用常量引用函数 -->

    <h3>计数值：{{ $store.state.counter }}</h3>
    <button @click="addCounter()" class="animated-fill-button">+10</button>
    <button @click="subCounter()" class="animated-fill-button">-10</button>
  </div>
</template>
